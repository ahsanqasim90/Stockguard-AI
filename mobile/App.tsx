...
function Forecast({ canWrite }: { canWrite: boolean }) {
  ...
  return <ScrollView contentContainerStyle={s.content}><Text style={s.title}>AI forecast</Text><Text style={s.muted}>Demand from your uploaded sales history</Text>
    <Card><Text style={s.cardTitle}>Sales series</Text><ScrollView horizontal showsHorizontalScrollIndicator={false}>{series.map(item => <Pressable key={`${item.storeId}-${item.productId}`} onPress={() => setSelected(item)} style={[s.chip, selected?.storeId === item.storeId && selected.productId === item.productId && s.chipActive]}><Text style={s.strong}>{item.productName} · {item.storeCode}</Text></Pressable>)}</ScrollView>
      ...
      {canWrite && <Button title={busy ? 'Training & comparing…' : 'Run live comparison'} onPress={generate} disabled={busy || !selected} />}</Card><ErrorText text={error} />
    {run ? <><Card><Text style={s.cardTitle}>{run.productName}</Text><Text style={s.muted}>{run.sku} · {run.storeCode}</Text><Text style={s.big}>{number(run.forecastTotal)}</Text><Text style={s.muted}>Predicted units over {run.horizon} days</Text><Text style={s.muted}>Selected: {run.model.replaceAll('_', ' ')} v{run.modelVersion}</Text><Text style={s.muted}>Last actual: {run.latestActualDate} · starts {run.forecastStartDate}</Text><Text style={s.muted}>Backtest MAE {run.backtest.mae.toFixed(2)} · RMSE {run.backtest.rmse.toFixed(2)}</Text>{run.modelSelection ? <Text style={s.note}>{run.modelSelection}</Text> : null}</Card><Card><Text style={s.cardTitle}>Live model comparison</Text>{run.comparisons?.map(item => <View style={s.row} key={`${item.model}:${item.modelVersion}`}><View><Text style={s.strong}>{item.model.replaceAll('_', ' ')}{item.selected ? ' · selected' : ''}</Text><Text style={s.muted}>MAE {item.mae.toFixed(3)} · RMSE {item.rmse.toFixed(3)}</Text></View></View>)}</Card><Card><Text style={s.cardTitle}>Daily predictions</Text>{run.predictions.map(point => <View style={s.row} key={point.date}><Text style={s.muted}>{point.date}</Text><Text style={s.strong}>{point.forecast_sales.toFixed(2)} units</Text></View>)}</Card></> : <Text style={s.muted}>No saved forecast yet.</Text>}
    <Text style={s.note}>Production XGBoost runs live for mapped store and product IDs. Custom IDs and shorter histories use the seasonal fallback automatically.</Text>
  </ScrollView>;
}
