...
const capabilities = [
  [BrainCircuit, "AI demand forecasting", "Live Linear Regression, ARIMA, Random Forest and XGBoost candidates turn sales history into a measured demand plan."],
...
const faqItems = [
  ...
  ["Are these live business forecasts?", "Yes. Uploaded business sales are evaluated live by the Python service and the production candidate; the selected run is saved to MongoDB."],
];
...
function MiniDashboard({ expanded = false }) {
  return <div className={`mini-dashboard ${expanded ? "expanded" : ""}`}>
    ...
    <div className="mini-kpis">
      <div><span>30-day demand</span><strong>1,830.8</strong><small>units forecast</small></div>
  ...
  return <div className="landing" id="top">
    ...
    <main>
      ...
      <section className="landing-section suite-section"><div className="section-intro"><span>Full feature suite</span><h2>Everything you need to <em>plan smarter</em></h2></div><div className="suite-grid"><div className="suite-list">{[
        ["Multi-store forecasting", "Explore 30-day demand across every supported store and product."],
      ...
      <section className="landing-section workflow-section" id="workflow"><div className="section-intro"><span>Simple 3-step workflow</span><h2>From raw data to <em>AI insights</em></h2></div><div className="workflow-grid">{[[FileUp,"01","Upload sales data","Import CSV sales history and validate required fields."],[BrainCircuit,"02","Compare live models","The signed Python service measures three proposal models while the Node API evaluates the production candidate."],[PackageCheck,"03","Act on insights","Use the automatically selected lowest-error forecast for inventory planning."]].map(([Icon,number,title,copy]) => <article key={number}><div className="step-icon"><span>{number}</span><Icon size={24} /></div><h3>{title}</h3><p>{copy}</p></article>)}</div></section>

      <section className="model-showcase" id="technology"><div className="model-copy"><div className="landing-pill violet"><BrainCircuit size={14} /> Live forecasting engine</div><h2>Four measured candidates.<br /><span>One selected forecast.</span></h2><p>Every business run evaluates Linear Regression, ARIMA and Random Forest alongside the available production candidate using the same holdout window.</p><div className="score-row"><div><b>Python ML</b><span>Proposal models</span><strong>3 live</strong></div><div><b>XGBoost</b><span>Production candidate</span><strong>v1.0.0</strong></div><div><b>Selection</b><span>Holdout metrics</span><strong>MAE + RMSE</strong></div></div></div><div className="model-numbers"><article><Database /><strong>8.17M</strong><span>XGBoost training rows</span></article><article><Layers3 /><strong>11,626</strong><span>mapped series</span></article><article><CircleGauge /><strong>4</strong><span>live candidates</span></article><article><Activity /><strong>30 days</strong><span>max forecast horizon</span></article></div></section>
      ...
      <section className="architecture-section"><div className="section-intro"><span>Proposal-aligned platform</span><h2>A clear, scalable <em>system architecture</em></h2></div><div className="architecture-grid"><article><div><Users /></div><span>Client layer</span><h3>React + React Native</h3><p>Responsive web dashboard and Expo mobile application on one authenticated API.</p></article><ChevronRight /><article><div><Layers3 /></div><span>Business layer</span><h3>Node.js + Express</h3><p>Authentication, product management, sales uploads and automatic model selection.</p></article><ChevronRight /><article><div><Database /></div><span>Data layer</span><h3>MongoDB</h3><p>Users, products, sales, inventory, forecasts and comparison metrics.</p></article><ChevronRight /><article><div><BrainCircuit /></div><span>Intelligence layer</span><h3>Python ML + XGBoost</h3><p>Signed live service for Linear Regression, ARIMA, Random Forest and production XGBoost inference.</p></article></div></section>
